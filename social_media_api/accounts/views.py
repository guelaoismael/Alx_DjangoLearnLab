...
class FollowUser(mixins.CreateModelMixin,generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            user_to_follow = User.objects.get(pk=self.kwargs['user_id'])
        except User.DoesNotExist:
            return Response({"error": "The user that you want to follow doesn't exist"}, status=status.HTTP_404_NOT_FOUND)
...
class UnfollowUser(mixins.CreateModelMixin,generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            user_to_unfollow = User.objects.get(pk=self.kwargs['user_id'])
